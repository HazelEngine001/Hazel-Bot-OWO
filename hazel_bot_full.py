...
async def cf(ctx, amount: int, choice: str = None):
    ...
    if choice == result:
        ...
        final_msg = (f"{user_name} đặt cược {amount:,} icoin và chọn {choice.capitalize()}\n\n"
                     f"Kết quả là {icon_result} và bạn **thắng {win_amount:,} icoin**!")
    else:
        update_balance(user_id, -amount)
        final_msg = (f"{user_name} đặt cược {amount:,} icoin và chọn {choice.capitalize()}\n\n"
                     f"Kết quả là {icon_result} và bạn mất hết tiền cược.")
...
async def cfall(ctx):
    user_id = ctx.author.id
    user_name = f"**{ctx.author.display_name}**"  # in đậm tên không tag
    create_user(user_id)
...
import os
bot.run(os.getenv("MTM1NTE0MTU3NjQ2ODIwMTU1Mw.GdB8DZ.iv0VW_HPheuxoOKKBR3lg2qvayXjVSU09mgtxg"))
